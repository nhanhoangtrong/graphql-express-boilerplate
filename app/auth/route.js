...
const { localStrategy } = require('./authStrategies');
    ...
    .post('/signup', async (req, res, next) => {
        ...
        try {
            ...
            if (!validator.isLength(password + '', { min: 8, max: 32 })) {
                throw new SignupError('Errors on registering.', {
                    password: 'Password must be in range (8-32) characters.',
                });
            }
            if (
                !validator.isLength(first_name + '', { min: 1, max: 100 }) ||
                !validator.isLength(last_name + '', { min: 1, max: 100 })
            ) {
                throw new SignupError('Errors on registering.', {
